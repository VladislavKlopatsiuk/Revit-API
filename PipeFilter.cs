...
namespace Спека_труб_горизонтальных_и_вертикальных
{
    ...
    public class PipeFilter : IExternalCommand
    {
        public Result  Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            FilteredElementCollector linkedFileCollector = new FilteredElementCollector(doc).OfClass(typeof(RevitLinkInstance));
...

            {
                ...
                foreach (var curveElement in pipe)
                {
                    ...
                    List<double> percentage_list = new List<double>();    //удалить потом                

                    bool procentage_general = false;                              
                                        
                    if (contain_value && contain_value_pipe_option)
                    {
                        ElementIntersectsElementFilter intersects_element_filter = new ElementIntersectsElementFilter(curveElement);

                          if (intersects_element_filter != null)
                            {                           

                        IList<Element> other_elrments = new FilteredElementCollector(doc).WherePasses(intersects_element_filter).ToElements();                        

                        List<Element> other_elrments_form_links = GetIntersectsFromLinkedDocuments(doc, curveElement, linkedFileCollector);

                        List<Element> all_intersect_other_elements = other_elrments.Concat(other_elrments_form_links).ToList();

                        Options options = new Options();

                        GeometryElement pipe_geometry = curveElement.get_Geometry(options);
                        
                        foreach (GeometryObject geometryObject_pipe in pipe_geometry)
                        {
                                    Solid pipe_solid = geometryObject_pipe as Solid;

                                    double pipe_volume = pipe_solid.Volume;

                                    List<double> other_elements_volume_list = new List<double>();

                            foreach (Element other_element_instance in all_intersect_other_elements)
                            {
                                Options options_other = new Options();

                                GeometryElement other_gepmetry_element_instance = other_element_instance.get_Geometry(options_other);                                

                                foreach (GeometryObject geometryObject_other_element in other_gepmetry_element_instance)
                                {
                                    Solid other_element_solid = geometryObject_other_element as Solid;

                                    if (other_element_solid != null && pipe_solid != null)
                                    {
                                            try
                                            {
                                                Solid intersectionSolid = BooleanOperationsUtils.ExecuteBooleanOperation(pipe_solid, other_element_solid, BooleanOperationsType.Intersect);
                                                if (intersectionSolid != null)
                                                {
                                                    double intersection_volume = intersectionSolid.Volume;

                                                    other_elements_volume_list.Add(intersection_volume);
                                                }
                                            }
                            ...
                            if (other_elements_volume_list.Any())
                            {
                                double percentage = (other_elements_volume_list.Max() / pipe_volume) * 100;

                                percentage_list.Add(percentage);

                                if (percentage < 50)
                                {
                                    procentage_general = true;
                                }

                                other_elements_volume_list.Clear();

                                //var simpleform = new SimpleForm(percentage_list);
                                //simpleform.ShowDialog();

                                percentage_list.Clear();
                            }                            
                            else
                            {
                                procentage_general = true;
                    ...
                    if (contain_value && contain_value_pipe_option && procentage_general)
                    {
            ...
            TaskDialog.Show("Запись о положении труб в пространстве", "Отработано");
            return Result.Succeeded;
        }
        private List<Element> GetIntersectsFromLinkedDocuments(Document doc, Element pipe, FilteredElementCollector link)
        {
            List<Element> linkElement = new List<Element>();            

            FilteredElementCollector linkedFileCollector = link;

            ElementIntersectsElementFilter intersects_element_filter = new ElementIntersectsElementFilter(pipe);

            foreach (RevitLinkInstance linkInstance in linkedFileCollector)
            {
                Document linkedDoc = linkInstance.GetLinkDocument();

                if (linkedDoc != null)
                {
                    IList<Element> other_elements = new FilteredElementCollector(linkedDoc).OfCategory(BuiltInCategory.OST_Walls).OfCategory(BuiltInCategory.OST_Floors).WherePasses(intersects_element_filter).ToElements();

                    foreach (Element otherElement in other_elements)
                    {
                        linkElement.Add(otherElement);
                    }
                }
            }

            //var simpleform2 = new SimpleForm(linkElement);
            //simpleform2.ShowDialog();
 ...
 //foreach (Element other_instance_element_link in other_elrments)
                    //{
                    //    GeometryElement geometry = other_instance_element_link.get_Geometry(options);
                    //    foreach (GeometryObject geometry_object in geometry)
                    //    {
                    //        Solid instance_solid = geometry_object as Solid;
                    //        if (instance_solid != null)
                    //        {
                    //            linkSolids.Add(instance_solid);
                    //        }
                    //    }
                    //}